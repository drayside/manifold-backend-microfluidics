...
import org.manifold.compiler.back.microfluidics.smt2.QF_NRA;
...
public class PythagoreanLengthRuleStrategy extends LengthRuleStrategy {
  ...
  @Override
  public List<SExpression> _translate(Schematic schematic, ProcessParameters processParams,
    ...
    for (NodeValue n1 : schematic.getNodes().values()) {
      for (NodeValue n2 : schematic.getNodes().values()) {
        ...
        if (channel != null) {
          exprs.add(_generateLengthAssertion(schematic, n1, n2, channel));
        }
      }
    }
    return exprs;
  ...
  private SExpression _generateLengthAssertion(Schematic schematic,
    ...
    SExpression sideA = QF_NRA.subtract(n1x, n2x);
    SExpression ASquared = QF_NRA.pow(sideA, new Numeral(2));
    SExpression sideB = QF_NRA.subtract(n1y, n2y);
    SExpression BSquared = QF_NRA.pow(sideB, new Numeral(2));
    SExpression ASquaredPlusBSquared = QF_NRA.add(ASquared, BSquared);
    SExpression CSquared = QF_NRA.pow(chLen, new Numeral(2));
    return QF_NRA.equal(ASquaredPlusBSquared, CSquared);
  }
